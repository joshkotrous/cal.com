...
class RoutingEventsInsights {
  ...
  static async getFailedBookingsByRoutingFormGroup({
    ...
    if (routingFormId) {
      // Use parameterized query for routingFormId to prevent SQL injection
      teamConditions.push(Prisma.sql`f.id = ${routingFormId}`);
    }

    // Build the whereClause using parameterized queries for all conditions
    let whereClause;
    if (teamConditions.length) {
      // Join conditions, ensuring parameterized conditions are handled correctly
      const sqlParts = [];
      for (const cond of teamConditions) {
        if (typeof cond === 'string') {
          sqlParts.push(Prisma.raw(cond));
        } else {
          sqlParts.push(cond);
        }
      }
      // Join with AND, but ensure parameterized conditions are not interpolated as raw strings
